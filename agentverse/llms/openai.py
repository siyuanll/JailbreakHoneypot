...
class OpenAIChat(BaseChatModel):
    ...
    def generate_response(self, prompt: str, chat_memory: List[Message], final_prompt: str) -> LLMResult:
        ...
        try:
            if openai.api_type == "azure":
                response = client.chat.completions.create(engine="gpt-3.5-turbo", messages=messages, **self.args.dict())
            else:
    ...
    async def agenerate_response(self, prompt: str, chat_memory: List[Message], final_prompt: str) -> LLMResult:
        ...
        try:
            if openai.api_type == "azure":
                response = await aclient.chat.completions.create(engine="gpt-3.5-turbo", messages=messages, **self.args.dict())
            else:
